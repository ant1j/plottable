...
class RichTable:
    def __init__(
        ...
        # self.column_name_to_idx = self._init_column_name_to_index()
    ...
    def _init_column_name_to_index(self):
        """
        FIXME Never used
        """
        return {col: i for i, col in enumerate(self.column_names)}
    ...
    def _get_column_titles(self) -> List[str]:
        """Returns a List of Column Titles.
                ...
                Returns:
        1            List[str]: List of Column Titles
        """
        return [
            self.column_definitions[col].get("title", col) for col in self.column_names
        ]
    ...
    def _get_non_group_colnames(self) -> set[str]:
        ...
        raise Exception("Was actually called")
    ...
    def _get_row(self, idx: int, content: List[str | Number]) -> Row:
        ...
        for col_idx, (colname, width, _content, x) in enumerate(
            ...
            col_def = self.column_definitions[colname]
            # FIXME Should push everything to create_cell that should holds all the logic to build cells
